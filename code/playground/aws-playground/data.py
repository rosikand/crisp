...
# global config (see https://github.com/karpathy/nanoGPT/blob/master/train.py#L6 for why) 

s3_bucket = 'dummynaturalist'


class S3CrispDataset(Dataset):
    def __init__(self, bucket, csv_file_path, images_dir_path, remote_sensing_dir_path, data_split="train"):
        """
        ...
        self.df = self.read_s3_csv(self.csv_file)
    ...
    def read_s3_csv(self, filepath):
        obj = self.s3_client.get_object(Bucket=self.s3_bucket, Key=filepath)
        df = pd.read_csv(obj['Body'])
        return df
    

    def load_s3_image(self, filepath):
       # TODO 
       pass
